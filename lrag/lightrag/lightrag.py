...
class LightRAG:
    ...
    vector_storage: str = field(default="WeaviateDBVectorStorage")
    ...
    async def ainsert_custom_chunks(
        ...
        try:
            ...
            # Skip duplicate check or overwrite
            _add_doc_keys = await self.full_docs.filter_keys({doc_key})
            if not _add_doc_keys:
                logger.warning(f"[DEBUG] Document {doc_key} already exists. Overwriting...")
                await self.full_docs.drop()
                await self.text_chunks.drop()
                _add_doc_keys = {doc_key}

            new_docs = {k: v for k, v in new_docs.items() if k in _add_doc_keys}
            if not new_docs:
                logger.error("[ERROR] Failed to prepare new_docs for insertion")
                return

            update_storage = True
            logger.info(f"[DEBUG] Inserting {len(new_docs)} docs")
            ...
            if not inserting_chunks:
                logger.warning("[DEBUG] All chunks are duplicates. Overwriting...")
                await self.text_chunks.drop()
                inserting_chunks = {
                    compute_mdhash_id(chunk_text, prefix="chunk-"): {
                        "content": chunk_text,
                        "full_doc_id": doc_key,
                        "tokens": len(self.tokenizer.encode(chunk_text)),
                        "chunk_order_index": index,
            ...
            logger.info(f"[DEBUG] Inserting {len(inserting_chunks)} chunks")
            tasks = [
                self.chunks_vdb.upsert(inserting_chunks),
                self._process_entity_relation_graph(inserting_chunks),
                self.full_docs.upsert(new_docs),
            ...
            # await asyncio.gather(*tasks)
            # logger.info(f"[DEBUG] Stored {len(inserting_chunks)} chunks and {len(new_docs)} docs")
            try:
                await asyncio.gather(*tasks)
                logger.info(f"[DEBUG] Successfully stored {len(inserting_chunks)} chunks and {len(new_docs)} docs")
                # Verify Weaviate storage
                weaviate_count = await self.chunks_vdb.query("", top_k=100)
                logger.info(f"[DEBUG] Weaviate contains {len(weaviate_count)} objects after upsert")
            except Exception as e:
                logger.error(f"[ERROR] Failed to upsert chunks: {e}", exc_info=True)
                raise
        except Exception as e:
            logger.error(f"[ERROR] Error in ainsert_custom_chunks: {e}", exc_info=True)
            raise
        finally:
            if update_storage:
                await self._insert_done()
    ...
    async def apipeline_enqueue_documents(
        ...
        Args:
            ...
            file_paths: list of file paths corresponding to each document, used for citation
        """
        logger.info(f"[DEBUG] apipeline_enqueue_documents called with {len(input) if isinstance(input, list) else 1} document(s)")
        logger.debug(f"[DEBUG] Input sample: {input[:200] if isinstance(input, str) else input[:2]}")
        logger.debug(f"[DEBUG] IDs: {ids}, File paths: {file_paths}")
        ...
        logger.info(f"[DEBUG] After duplicate removal: {len(contents)} unique contents")
        # 3. Generate document initial status
        ...
        logger.info(f"[DEBUG] Checking {len(all_new_doc_ids)} document IDs against doc_status")
        # Exclude IDs of documents that are already in progress
        unique_new_doc_ids = await self.doc_status.filter_keys(all_new_doc_ids)
        logger.info(f"[DEBUG] Found {len(unique_new_doc_ids)} unique new document IDs")
